...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setLoading(true); setError(""); const data = new FormData(event.currentTarget); try { const token = await api.login(String(data.get("username")), String(data.get("password"))); setAccessToken(token.access_token); router.push(params.get("next") ?? "/"); } catch (e) { setError(e instanceof Error ? e.message : "Помилка входу"); } finally { setLoading(false); } }
