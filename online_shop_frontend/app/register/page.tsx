...
export default function RegisterPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await api.register({ username, email: String(data.get("email")), password });
  ...
  return (
    <form onSubmit={submit} className="mx-auto max-w-md space-y-4 rounded-2xl bg-white p-6 shadow">
      ...
      <input required name="email" type="email" placeholder="Email" className="w-full rounded border p-3" />
      <input required name="password" type="password" minLength={6} placeholder="Пароль" className="w-full rounded border p-3" />
